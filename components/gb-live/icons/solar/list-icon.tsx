...
import { LIST_ICON_CX, LIST_ICON_CY, ICON_LIST_WIDTH } from "../list-icons";
...
const SolarListIcon: React.FC<SolarListIconProps> = ({ cycleSeconds }) => {
  const opacity = React.useMemo(() => calculateSolarOpacity(cycleSeconds), [cycleSeconds]);
  const c = useClock();
  ...
  return <Circle cx={LIST_ICON_CX} cy={LIST_ICON_CY} r={r} color={SUN_COLOR} opacity={opacity} />;
};
